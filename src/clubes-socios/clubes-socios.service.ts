...
@Injectable()
export class ClubesSociosService {
  constructor(
  ...
  async addMemberClub(clubId: string, socioId: string): Promise<ClubEntity> {
  ...
  async updateMembersFromClub(
    ...
    for (let i = 0; i < socios.length; i++) {
      const artwork: SocioEntity = await this.socioRepository.findOne({
